...
:- true pred init(N,_1,_2,_3,_4,_5)
   : ( mshare([[_5]]),
       ground([N,_1,_2,_3,_4]), linear(_5) )
   => ( mshare([[_5]]),
        ground([N,_1,_2,_3,_4]), linear(_5) ).
...
randomize(In,[X|Out],Rand) :-
    true((mshare([[In],[X],[Out],[Lin],[Rand1],[N],[In1]]),ground([Rand]),linear(In),linear(X),linear(Out),linear(Lin),linear(Rand1),linear(N),linear(In1);mshare([[In],[X],[Out],[Lin],[Rand1],[N],[In1]]),ground([Rand]),linear(X),linear(Out),linear(Lin),linear(Rand1),linear(N),linear(In1))),
...
:- true pred list_to_length(List,Len)
   : ( mshare([[List],[Len]]),
       linear(List), linear(Len) )
   => ( mshare([[List]]),
        ground([Len]) ).

list_to_length(List,Len) :-
    true((mshare([[List],[Len]]),linear(List),linear(Len);mshare([[List],[Len]]),linear(Len))),
...
list_to_length([_1|T],L0,L) :-
    true((mshare([[L],[_1],[_1,T],[T],[L1]]),ground([L0]),linear(L),linear(L1);mshare([[L],[_1],[T],[L1]]),ground([L0]),linear(L),linear(_1),linear(T),linear(L1))),
    L1 is L0+1,
    true((mshare([[L],[_1],[_1,T],[T]]),ground([L0,L1]),linear(L);mshare([[L],[_1],[T]]),ground([L0,L1]),linear(L),linear(_1),linear(T))),
    list_to_length(T,L1,L),
    true((mshare([[_1],[_1,T],[T]]),ground([L0,L,L1]);mshare([[_1],[T]]),ground([L0,L,L1]),linear(_1))).

:- true pred split(N,_A,X,Xs)
   : ( mshare([[_A],[X],[Xs]]),
       ground([N]), linear(X), linear(Xs) )
...
:- true pred investigate(_A,_1)
   : ( (_1=[[star(_B),_C,star(_D),_C,a,star(_B),a,star(_D),star(_B)],[star(_B),star(_D),star(_D),star(_B),[star(_B)],[star(_D)]],[_E,_F,star(_G),[b,a],star(_H),_I,_J]]),
       ...
       linear(_B), linear(_C), linear(_D), linear(_E), linear(_F), linear(_G), linear(_H), linear(_I), linear(_J) )
...
:- true pred get_pats(Npats,Ipats,Result)
   : ( mshare([[Result]]),
       ground([Npats,Ipats]), linear(Result) )
...
:- true pred get_pats(N,_1,Ys,_2)
   : ( mshare([[Ys]]),
       ground([N,_1,_2]), linear(Ys) )
   => ground([N,_1,Ys,_2]).

:- true pred get_pats(N,_1,Ys,_2)
   : ( (_1=_2),
       mshare([[Ys]]),
       ground([N,_1]), linear(Ys) )
   => ground([N,_1,Ys]).
