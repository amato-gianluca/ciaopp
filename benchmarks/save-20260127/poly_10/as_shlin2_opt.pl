...
:- true pred poly_add(Poly,C,_A)
   : ( mshare([[_A]]),
       ground([Poly,C]), linear(_A), shlin2([([_A],[_A])]) )
   => ground([Poly,C,_A]).

:- true pred poly_add(Poly,C,_A)
   : ( (Poly=poly(z,[term(1,1)])),
       mshare([[_A]]),
       ground([C]), linear(_A), shlin2([([_A],[_A])]) )
   => ground([C,_A]).
...
:- true pred term_add(Terms1,X,_A)
   : ( (X=[term(_C,_D)|_B]),
       mshare([[_A]]),
       ground([Terms1,_B,_C,_D]), linear(_A), shlin2([([_A],[_A])]) )
   => ground([Terms1,_A,_B,_C,_D]).

:- true pred term_add(Terms1,X,_A)
   : ( mshare([[_A]]),
       ground([Terms1,X]), linear(_A), shlin2([([_A],[_A])]) )
   => ground([Terms1,X,_A]).
...
:- true pred add_to_order_zero_term(Terms,C2,_A)
   : ( mshare([[_A]]),
       ground([Terms,C2]), linear(_A), shlin2([([_A],[_A])]) )
   => ground([Terms,C2,_A]).

:- true pred add_to_order_zero_term(Terms,C2,_A)
   : ( (C2=poly(_B,_C)),
       mshare([[_A]]),
       ground([Terms,_B,_C]), linear(_A), shlin2([([_A],[_A])]) )
...
:- true pred poly_exp(N,_1,Result)
   : ( mshare([[Result]]),
       ground([N,_1]), linear(Result), shlin2([([Result],[Result])]) )
   => ground([N,_1,Result]).

:- true pred poly_exp(N,_1,Result)
   : ( (N=10),
       mshare([[Result]]),
       ground([_1]), linear(Result), shlin2([([Result],[Result])]) )
   => ground([_1,Result]).
...
poly_exp(N,Poly,Result) :-
    true((
        ...
        linear(M),
        linear(Part),
        shlin2([([Result],[Result]),([M],[M]),([Part],[Part])])
    )),
    M is N>>1,
    true((
        mshare([[Result],[Part]]),
        ...
        linear(Result),
        linear(Part),
        shlin2([([Result],[Result]),([Part],[Part])])
    )),
    N is M<<1,
    !,
    true((
...
:- true pred poly_mul(P,C,_A)
   : ( (P=C),
       mshare([[_A]]),
       ground([P]), linear(_A), shlin2([([_A],[_A])]) )
   => ground([P,_A]).

:- true pred poly_mul(P,C,_A)
   : ( mshare([[_A]]),
       ground([P,C]), linear(_A), shlin2([([_A],[_A])]) )
   => ground([P,C,_A]).
...
:- true pred term_mul(_A,_1,Terms)
   : ( mshare([[Terms]]),
       ground([_A,_1]), linear(Terms), shlin2([([Terms],[Terms])]) )
   => ground([_A,_1,Terms]).
...
term_mul([Term|Terms1],Terms2,Terms) :-
    true((
        ...
        linear(PartA),
        linear(PartB),
        shlin2([([Terms],[Terms]),([PartA],[PartA]),([PartB],[PartB])])
    )),
    ...
    true((
        ...
        linear(Terms),
        linear(PartB),
        shlin2([([Terms],[Terms]),([PartB],[PartB])])
    )),
...
:- true pred single_term_mul(_A,_1,_B)
   : ( (_1=term(_C,_D)),
       mshare([[_B]]),
       ground([_A,_C,_D]), linear(_B), shlin2([([_B],[_B])]) )
   => ground([_A,_B,_C,_D]).

:- true pred single_term_mul(_A,_1,_B)
   : ( mshare([[_B]]),
       ground([_A,_1]), linear(_B), shlin2([([_B],[_B])]) )
   => ground([_A,_1,_B]).
...
mul_through([term(E,Term)|Terms],Poly,[term(E,NewTerm)|NewTerms]) :-
    true((
        ...
        linear(NewTerms),
        linear(NewTerm),
        shlin2([([NewTerms],[NewTerms]),([NewTerm],[NewTerm])])
    )),
