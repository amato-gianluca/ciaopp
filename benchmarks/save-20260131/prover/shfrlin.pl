...
:- true pred problem(_A,_B,_C)
   : ( mshare([[_A],[_B],[_C]]),
       var(_A), var(_B), var(_C), linear(_A), linear(_B), linear(_C) )
   => ground([_A,_B,_C]).
...
:- true pred expand(_1,_A,New)
   : ( (_A=fs(_B,[_1&_F|_E],_C,_D)),
       mshare([[New]]),
       var(New), ground([_1,_B,_C,_D,_E,_F]), linear(New) )
   => ground([_1,New,_B,_C,_D,_E,_F]).

:- true pred expand(_1,_A,New)
   : ( mshare([[New]]),
       var(New), ground([_1,_A]), linear(New) )
   => ground([_1,_A,New]).
...
:- true pred extend(Exp,_1,Neg,_2,_3,Set)
   : ( (_3=fs(_A,_B,Neg,_2)),
       mshare([[_2],[Set]]),
       var(_2), var(Set), ground([Exp,_1,Neg,_A,_B]), linear(_2), linear(Set) )
   => ( mshare([[_2]]),
        ground([Exp,_1,Neg,Set,_A,_B]) ).
