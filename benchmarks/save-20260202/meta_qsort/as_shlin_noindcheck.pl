...
interpret(G,_1) :-
    ...
    var(G),
    !,
    true((fails(_);mshare([[G],[_1]]),linear(G))),
    fail,
    true(fails(_)).
interpret((A,B),Rest) :-
    !,
    true((mshare([[Rest],[A],[A,B],[B],[Rest0]]),linear(Rest0);mshare([[Rest],[Rest0]]),ground([A,B]),linear(Rest0))),
    interpret(A,Rest0),
...
interpret(G,_1) :-
    true((mshare([[G],[_1],[Body]]),linear(Body);mshare([[_1],[Body]]),ground([G]),linear(Body))),
    define(G,Body),
...
interpret_disjunction((A->B),_1,Rest) :-
    true((mshare([[_1],[_1,A],[_1,A,B],[_1,B],[Rest],[A],[A,B],[B],[Rest0]]),linear(Rest0);mshare([[Rest],[A],[A,B],[B],[Rest0]]),ground([_1]),linear(Rest0);mshare([[Rest],[A],[Rest0]]),ground([_1,B]),linear(Rest0);mshare([[Rest],[Rest0]]),ground([_1,A,B]),linear(Rest0))),
    interpret(A,Rest0),
    !,
