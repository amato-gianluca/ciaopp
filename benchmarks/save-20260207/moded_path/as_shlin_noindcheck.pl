...
path(X,X,one).
path(X,Y,C) :-
    true((mshare([[X],[C],[Z],[A],[B]]),ground([Y]),linear(Z),linear(A),linear(B);mshare([[C],[Z],[A],[B]]),ground([X,Y]),linear(Z),linear(A),linear(B))),
    edge(X,Z,A),
    true((mshare([[X],[C],[Z],[B]]),ground([Y,A]),linear(B);mshare([[C],[Z],[B]]),ground([X,Y,A]),linear(B))),
    path(Z,Y,B),
