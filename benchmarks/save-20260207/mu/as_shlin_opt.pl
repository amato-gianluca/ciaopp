...
theorem(R,Depth,[[N|R]|P]) :-
    ...
    theorem(S,D,P),
    true((mshare([[R],[N]]),ground([Depth,P,D,S]),linear(R),linear(N);mshare([[N]]),ground([R,Depth,P,D,S]),linear(N))),
    rule(N,S,R),
...
:- true pred rule(_A,S,R)
   : ( mshare([[_A],[R]]),
       ground([S]), linear(_A), linear(R) )
   => ground([_A,S,R]).

rule(1,S,R) :-
    true((mshare([[R]]),ground([S]),linear(R);ground([S,R]))),
    rule1(S,R),
    true(ground([S,R])).
rule(2,S,R) :-
    true((mshare([[R]]),ground([S]),linear(R);ground([S,R]))),
...
rule1([i],[i,u]).
rule1([H|X],[H|Y]) :-
    true((mshare([[Y]]),ground([H,X]),linear(Y);ground([H,X,Y]))),
    rule1(X,Y),
    true(ground([H,X,Y])).
...
rule2([m|X],[m|Y]) :-
    true((mshare([[Y]]),ground([X]),linear(Y);ground([X,Y]))),
    my_append(X,X,Y),
    true(ground([X,Y])).
...
:- true pred my_append(_A,X,_B)
   : ( mshare([[_B]]),
       ground([_A,X]), linear(_B) )
   => ground([_A,X,_B]).

my_append([],X,X).
my_append([A|B],X,[A|B1]) :-
    true((mshare([[B1]]),ground([X,A,B]),linear(B1);ground([X,A,B,B1]))),
    my_append(B,X,B1),
