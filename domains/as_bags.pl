...
:- doc(module,"
This module implements bags (multisets of terms) and is used by domains in the
as_* collection. A multiset is an *ordered* list of terms of the form `T-N`,
where `T` is a term and `N` is a natural number representing its multiplicity.
").
...
:- pred bag_from_set(+S, -B)
   ...
   + (not_fails, is_det)
   # "@var{B} is the bag corresponding to the set @var{S} where all elements
...
:- pred bag_from_list(+L, -B)
   : list * ivar => isbag(B)
   + (not_fails, is_det)
   # "@var{B} is the bag corresponding to the list @var{L} where the
   multiplicity of each element is the number of its occurrences in @var{L}.".
...
:- test bag_union([X-2, Y-3, hello-2, world-1], [Y-1, world-4, zz-1], S)
   => (S = [X-2, Y-4, hello-2, world-5, zz-1]) + (not_fails, is_det).
...
:- pred bag_projection(+B, +S, -Proj)
   ...
   + (not_fails, is_det)
   # "@var{Proj} is the projection of the bag @var{B} on the set of variables
   @var{S}.".
...
:- pred bag_vars(?T, -B)
   : term * ivar => isbag(B)
   + (not_fails, is_det)
   # "@var{B} is the bag of variables occuring in @var{T}, with multiplicities
