...
% Example 6.2 "Optimality in Goal Dependent Analysis of Sharing"
...
% Example 6.3 "Optimality in Goal Dependent Analysis of Sharing"

example2(t(U, V), H, K) :-
    % [H, K, U] is not possible here (need -fmgu_sh_optimize=on)
    nothing.

% Example 6.19 "Optimality in Goal Dependent Analysis of Sharing"
...
% Example 7.3 "Optimality in Goal Dependent Analysis of Sharing"

example4(X, Y, Z) :-
    example4bis(t(X), t(Y), t(Z)).
    % [X, Y, Z] is not possible here (need -fmgu_sh_optimize=on and -fas_use_match=on)

% We refine the result of the sharing analysis to overcome the low precision
% of forward unification in share.

:- trust pred example4bis(U, V, W): mshare([[U], [V], [W]]) => mshare([[U, V], [V, W]]).
